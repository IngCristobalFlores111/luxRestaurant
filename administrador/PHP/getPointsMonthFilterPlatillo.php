...
$query ="SELECT SUM(total) AS total,SUM(costo) AS costo,SUM(ganancia) AS ganancia,MONTH(fecha) AS mes,YEAR(fecha) AS ano FROM `tbventaplatillos` WHERE MONTH(fecha) BETWEEN $monthFrom AND $monthTo AND YEAR(fecha) BETWEEN $yearFrom AND $yearTo AND `idplatillo`=(SELECT idplatillo FROM tbplatillos WHERE nombre='$nombrePlatillo') GROUP BY mes,ano";
