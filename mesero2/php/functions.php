<?php
function actualizarVentasPlatillo($idPedido){

$query="SELECT tbplatillos.idplatillo AS idPlat,tbpedido_platillo.`cantidad`,
tbplatillos.precio,tbplatillos.costo FROM `tbpedido_platillo` 
INNER JOIN tbplatillos ON tbplatillos.idplatillo =
 tbpedido_platillo.`idplatillo` WHERE tbpedido_platillo.`idpedido` = ?";
$sql = createMysqliConnection();
$result = $sql->get_bind_results($query,array("i",$idPedido));
$query="";
foreach($result as $plat){
	$cantidad = (float)$plat['cantidad'];
	$idPlatillo = $plat['idPlat'];
	$total = (float)$plat['precio'] *$cantidad;
	$costo = (float)$plat['costo'] *$cantidad;
	$ganancia = $total-$costo;

	$query.="INSERT INTO `tbventaplatillos`
(`fecha`, `idplatillo`, `cantidad`, `total`, `costo`, `ganancia`) 
VALUES (NOW(),$idPlatillo,'$cantidad','$total','$costo','$ganancia')
ON DUPLICATE KEY UPDATE cantidad = cantidad +$cantidad,total = total +$total,
costo = costo + $costo,ganancia = ganancia + $ganancia;";
...
function descontarInventario($idPedido){
    $sql = createMysqliConnection();
    $query ="SELECT tb_platillo_insumos_prov_actual.idAlmacen,tbinsumos_platillo.cantidad AS cantidad_insumo,tbpedido_platillo.cantidad,tbinsumos_platillo.idInsumo,tbinsumos_platillo.idplatillo FROM tbinsumos_platillo INNER JOIN tbpedido_platillo ON tbpedido_platillo.idplatillo =tbinsumos_platillo.idplatillo INNER JOIN tb_platillo_insumos_prov_actual ON tb_platillo_insumos_prov_actual.idInsumo = tbinsumos_platillo.idInsumo WHERE tbpedido_platillo.idpedido = ? GROUP BY tbinsumos_platillo.idplatillo, tbinsumos_platillo.idInsumo";

$result = $sql->get_bind_results($query,array("i",$idPedido));
$query ="";
foreach($result as $insumo){
    $cantidad = (float)$insumo['cantidad']*$insumo['cantidad_insumo'];
    $idPlatillo = $insumo['idplatillo'];
    $idInsumo = $insumo['idInsumo'];
    $idAlmacen = $insumo['idAlmacen'];
    $query.="UPDATE `tbalmacen_insumos_actual` SET cantidad = cantidad - $cantidad
    WHERE idProveedor = (SELECT tb_platillo_insumos_prov_actual.idProveedor
      FROM tb_platillo_insumos_prov_actual
        WHERE tb_platillo_insumos_prov_actual.idPlatillo = $idPlatillo
        AND tb_platillo_insumos_prov_actual.idInsumo = $idInsumo )
        AND idInsumo = (SELECT tb_platillo_insumos_prov_actual.idInsumo
     FROM tb_platillo_insumos_prov_actual WHERE
       tb_platillo_insumos_prov_actual.idPlatillo = $idPlatillo
      AND tb_platillo_insumos_prov_actual.idInsumo = $idInsumo);";
...
$queryUpdate2 ="";
$query2 = "SELECT cantidad,`idplatillo` FROM `tbpedido_platillo` WHERE `idpedido` = ?";
$inventarioUnidad = $sql->get_bind_results($query2,array("i",$idPedido));
foreach($inventarioUnidad as $inventario){
//$almacenProvActua = ""
$idP = $inventario['idplatillo'];
$c = $inventario['cantidad'];
$queryUpdate2="UPDATE `tbinventario_unidad` SET `cantidad` = `cantidad` - $c WHERE `idPlatillo` = $idP AND `actual` = 1;";
}
if($queryUpdate2!=''){
	$sql->ejecutarNoQuery($queryUpdate2);
}
$respuesta = array();
$errorLog = $sql->getErrorLog();
if(count($errorLog)>0){
